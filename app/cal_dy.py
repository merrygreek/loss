...
class data_sxdl():
    ...
    def day_sxdl(self,mc):
        k = []
        tq = self.df[self.df.用户名称== mc ][['时间','A相电压','B相电压','C相电压']].set_index('时间')
...
class draw_sxdl():
    ...
    def day_sxdl(self,x,mc,a,b,c,k):
        c = (
            ...
            .add_xaxis(x)
            .add_yaxis("电压A",a,is_smooth=True)
            .add_yaxis("电压B",b,is_smooth=True)
            .add_yaxis("电压C",c,is_smooth=True)
            .set_global_opts(
                tooltip_opts=opts.TooltipOpts(
                is_show=True, trigger="axis", axis_pointer_type="cross"
                ),
                title_opts={'text':mc,'subtext':None},
        ...
        line1 = Line().add_xaxis(x).add_yaxis("方差", k,yaxis_index=1,is_smooth=True,linestyle_opts=opts.LineStyleOpts(color="black", width=2, type_="solid")).set_series_opts(
        label_opts=opts.LabelOpts(is_show=False), )
        c.overlap(line1)
        return c
